---
name: today
description: "运行多市场每日股票监控系统，生成今日报告。包含A股+美股+期货异动检测、AI分析、假设追踪和Today日报。"
---

# /today - 每日股票监控

运行A股+美股+期货每日监控系统，生成今日报告。
...
2. 如果 pipeline 失败，检查错误原因：
   ...
   - TqSdk 获取期货失败 → 检查 `config.yaml` 中 `tqsdk.username/password` 是否填写
...
| 期货 | TqSdk (天勤量化) | `config.yaml` 中 `tqsdk.username/password` |
...
- 期货历史数据累积在 `output/futures_quote_history.json`
